...
const MessageContainer = () => {
  const isLoading = useRecoilValue(chatLoadingState);
  ...
  return (
    <Container ref={scrollRef}>
      {chatList.map((chat: ChatItem, index: number) => {
        if (chat.role === "user") {
          return <MyMessage key={index} content={chat.content} />;
        } else if (chat.role === "assistant") {
          return <BotMessage key={index} content={chat.content} isLoading={false} />;
        }
        return null;
      })}
